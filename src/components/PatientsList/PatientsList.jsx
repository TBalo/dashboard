...
const PatientsList = () => {
    ...
    ({
        ...
        color: "gray", className: "rounded-full  bg-slate-200 w-8 ",
    ...
    <div className='w-full mt-[8vh] px-5 md:mt-[5vh] lg:mt-[12vh] bg-[#FAFAFA] h-auto lg:h-full max-w-[80%] md:max-w-[90%] lg:max-w-[85%]'>
            ...
            <div className="w-[97.5%] mx-4 my-2 items-center flex justify-between bg-white rounded ">
                <div className='p-4 items-center'>Total Patients<span className='text-[#828282]'> (487)</span></div>
            ...
            <div className='w-full mx-4   max-md:hidden'>
                <table className=" w-[97.5%]  bg-white">
                    <thead className=' w-auto' style={{ height: "50px", borderBottom: "2px solid #E0E0E0" }}>
                    ...
                    <tbody className='bg-grey-light flex flex-col items-center justify-between overflow-y-scroll w-full' style={{ height: "54vh" }}>
                        {
                            data.map((item) => {
                                ...
                                return (
                                    ...
                                    <tr className=' h-12 flex w-full mb-4 text-center ' key={item.Name}>
                                        <td className='p-4 w-1/6'>{item.Name}</td>
                                        <td className='p-4 w-1/6'>{item.Diagnosis}</td>
                                        <td className='pt-4 w-1/6'>  <button type="button" className={background} >{item.Status} </button>  </td>
                                        <td className='p-4 w-1/6'>{item['Last Appointment']}</td>
                                        <td className='p-4 w-1/6'>{item['Next Appointment']}</td>
                                        <td className='p-4 w-1/6 text-4xl flex items-center  justify-center'>{item.Option}</td>
                                    </tr>
                                )
                            })
                        }
            ...
            <div className="w-full md:hidden overflow-x-scroll" style={{height:"70vh"}}>
                {data.map((item)=>{
                     if (item.Status === 'Recovered') {
                    ...
                    return(
                        ...
                        <div class="grid grid-flow-row auto-rows-max">
                            ...
                            <div className='whitespace nowrap max-w-full'>Last Appointment</div>
                            <div className='whitespace nowrap max-w-full'>Next Appointment</div>
                            <div className='whitespace nowrap max-w-full'>Option</div>
                        </div>
                        <div class="grid grid-flow-row auto-rows-max">
                            <div>{item.Name}</div>
                            <div>{item.Diagnosis}</div>
                            <div><button type="button" className={background}>{item.Status}</button> </div>
